...
class DataValidator:
    ...
    def validate_numeric(
        ...
        quality = DataQuality.EXCELLENT if len(errors) == 0 else DataQuality.POOR
    ...
    def validate_financial_metrics(
        ...
        return ValidationResult(
            is_valid=len(errors) == 0,
            quality=DataQuality.GOOD if len(errors) == 0 else DataQuality.FAIR,
            errors=errors,
            warnings=warnings,
    ...
    def validate_consistency(
        ...
        for key in current.keys():
            if key in historical[-1] if historical else None:
        ...
        return ValidationResult(
            is_valid=True,
            quality=DataQuality.GOOD if len(warnings) == 0 else DataQuality.FAIR,
            errors=[],
            warnings=warnings,
        ...
        for field in fields:
            if field in data and isinstance(data[field], (int, float)):
                if method == "minmax":
                    ...
                    normalized[field] = (data[field] - mean) / stdev if stdev != 0 else 0
            ...
            if values and metric in company_data:
                ...
                if isinstance(company_val, (int, float)):
                    ...
                    comparison_results[metric] = {
                        ...
                        "行业最低": min_val,
                        "行业百分位": percentile,
                        "相对均值": (company_val - mean) / mean if mean != 0 else 0,
                        "相对中位数": (company_val - median) / median if median != 0 else 0
                    }
