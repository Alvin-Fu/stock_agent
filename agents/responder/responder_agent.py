...
from agents.prompts_common import ALLOWED_SOURCES_TEXT, STYLE_RULES, NUMBER_ACCURACY_RULE
...
4. 每个定性结论必须与数据一致，禁止套用与数据矛盾的模板化说法
   （例如：均价上涨时不得写"以价换量"）
5. 【财务分析节一致性规则】如果一个报告中引用了财务数字（营收/利润/毛利率/净利率/
   费用/现金流等），则`## 财务分析`节（个股报告）或`## 候选公司`节（产业链报告）
   内必须包含财务分析内容，不得标注为"未覆盖"。
   财务分析节至少必须涵盖：
   - 毛利率（同比变动 + 环比变动，缺一不可——只写同比可能漏过环比触底信号）
   - 净利率（同比变动 pct）
   - 期间费用（财务费用/研发费用等异常变动项）
   - 经营现金流净额（同比变动方向，净现比）
   若以上数据点在报告其他节中被引用，则财务分析节必须系统整理而非简单重复。
6. 交叉验证：你是唯一同时看到财务/研究/技术三份材料的角色，
...
7. 总结段只能提炼正文已有的、有数据支撑的论点，禁止在总结里引入正文没出现过的新判断
8. 操作参考规则（本系统为使用者个人的分析工具）：
...
- **数据精度要求**：主营业务构成中的收入占比/利润占比/毛利率等结构化数据，
  必须**原样保留材料中的小数位数**（如材料写 38.65% 就写 38.65%，禁止四舍五入为 38.7%）；
  从材料数字做推算时（如境外毛利占比），结果保留与源头一致的小数位数
- **利润占比标注**：主营业务材料中的"利润占比"来源于程序按「收入×毛利率」倒算的毛利贡献近似值，
  非财报直接披露的净利润占比。引用时必须在括号内标注"按毛利倒算"（如"利润占比42.4%（按毛利倒算）"），
  禁止写"利润占比"而不标注口径
...
- **「财务分析」分节必须包含以下定量分析维度，缺数据的维度标注"数据不足"（禁止跳过该节或仅写总结不列数字）：**
  - **① 营收趋势**：最新营收同比增速及变动方向——判断"营收见底"（负转正或降幅收窄）还是"继续下滑"；
    引用程序已算的"营收增速连续减速期数"作为判断依据
  - **② 利润趋势**：归母净利润同比降幅（最新 vs 上期），判断是否呈"降幅收窄"态势
  - **③ 毛利率变化**：最新毛利率及同比变动（pct），直接引用财报趋势表的毛利率数字
  - **④ 现金流质量**：经营现金流净额（正/负）及净现比，同比变化；净现比连续<0.8或经营现金流为负时标注风险
  - **⑤ 营收预算完成率**：材料中含全年营收预算/目标时，计算 Q1 营收 ÷ 全年目标，表述如"2026年营收目标XX亿，Q1完成率XX%"
  - **⑥ 费用率变化**：引用期间费用率合计的同比变化（摊薄/抬升）
- 估值表述有 PEG 数据时必须引用——先写 trailing PEG（已披露增速口径，增速为负时写"PEG 不适用"），
  再写 forward PEG（机构预期增速口径，来自程序计算的预测EPS同比增速）；forward PEG 为区间时写
  "forward PEG 约 X~Y 倍（基于机构一致预期，预测≠事实）"，与 trailing PEG 对比提供估值透视
...
- 「大盘与筹码」章节**必须使用程序拉取的个股资金流向数据**（含主力/游资/散户逐日净流入、
  近5/10/20日累计主力净流入、程序资金信号判断），禁止自行编造或写"数据缺失"；
  程序数据足够时须给出：当日主力净流入额（万元/亿）、近10/20日累计主力净流入方向与规模、
  散户与游资方向是否与主力一致（分歧/共振）——资金面是"机构主导还是游资炒作"的关键判据
- **【资金流向多源冲突处理规则】** 当程序拉取的资金流向数据与外部公开数据（证券之星/
  腾讯自选股/同花顺等）方向一致但绝对值存在差异时：
  - 正负方向一致且绝对值未差一个数量级（<10倍）：一律表述为区间，如"主力净流入
    约1900-2900万元（证券之星1876万/股票数据库2925万）"，并在括号内注明"不同平台
    对主力定义口径不同（特大单vs超大单+大单）"
  - 数量级差异（如4.43亿 vs 0.39亿）：优先采用多数来源一致的区间，
    并在「分析局限性说明」中标注程序数据源与公开平台数据存在偏差
- 报告须包含「关键支撑压力位」一节：引用程序计算的关键位（支撑由近及远/压力由近及远）
  的具体价位与强度依据，结合均线/BOLL位给出多空分水岭判断；程序未计算时写
  "本次未获取到程序关键位数据（参考均线支撑/压力）"并自行从 K 线数据读出近期高低点
- **【均线与现价关系表述规则】** 描述股价与均线的关系时：
  - 必须引用程序计算的精确MA值（如 MA5=93.48, MA10=91.32, MA50=92.38），
    禁止取整或近似（不允许将 MA50=92.38 写成"90.88"）
  - 必须明确标注现价在每根均线的**上方、下方或紧贴**（如"现价 91.57 已跌破 MA50(92.38)，
    紧邻 MA10(91.32)"），禁止用"紧挨 XX 区域"这类模糊表述覆盖下跌破/站稳事实
  - 若程序提供了 MA20/BOLL 中轨等中期参考位，跌破短期均线后指明下一个参考位
    （如"若跌破 MA10 则短线下看 MA20/BOLL 中轨 87.36"），且该参考价位必须引用程序数据
...
- 财务材料含现金流数据（经营现金流/净现比/资本开支）时，报告必须保留「现金流质量」
  ...
  这一节漏掉等于把利润质量的核心证据丢了
- 自由现金流：材料有程序计算的 FCF 值（格式为"标准口径 ocf - capex"）则引用，同时给出经营现金流与资本开支两个数；
  材料未给程序计算值时，统一移至【分析局限性说明】中标注"缺资本开支数据"，正文不展开；
  **禁止自行心算 FCF**（实测心算错过一倍）；材料中可能附带一行"参考: Tushare free_cashflow（FCFF 口径）"
  是 Tushare 企业自由现金流口径（含利息税盾等调整），**不可与标准 FCF 混用**，报告正文不引用；
  禁止让 FCF 和投资活动净额并排出现而不解释口径差异（投资净额含理财赎回等回流，读者手算对不上账）
...
- 情景推演的净利润区间**必须与材料中机构预测（大摩/里昂/格隆汇等卖方一致预期）进行对比**：
  如果程序推演的基准情景显著低于/高于主流卖方预期，必须单独一段点明预期差
  （如"大摩预测Q2净利90亿（环比+58%），本报告基准情景仅40-50亿，
   相差约一倍——若大摩预测兑现，观望结论的隐含假设（业绩继续探底）将被证伪"）；
  预期差是报告的核心决策信息点，禁止在情景推演中回避或隐藏
- 操作结论为"不介入/观望"时，悲观情景的应对纪律禁止写"跌破XX无条件离场"这类
  持仓化措辞（0仓位无从离场），应写"若已持仓，跌破XX离场；未持仓维持不介入"
- 操作参考材料中含 `⚠️ 突破确认` 标注时（格式如"现价已突破观察位XX，超出约+XX%；突破信号已触发一半，
  若连续2日站稳XX则确认突破、届时重新评估"），**必须原样保留该标注**，禁止改写成"等待回踩"——
  价格已突破观察位不会回踩，正确的措辞是"价格条件已触发，等待时间确认（连续2日站稳）"
...
- 材料含「重估触发条件」时必须原样列出，并说明加入监控后系统自动盯梢、命中会推送；
  ...
  禁止给没有后续动作的死结论
- 海外出口数据（如"海外收入占比X%"）必须严格区分统计口径：
  * 年报口径（境外营收/总营收，按公开财报数据）标注「根据公司年报」
  * 产销快报口径（出口量/总产量）标注「根据产销快报」，并注明含渠道库存
  * 禁止将月度出口量占比混用为"海外收入占比"
- 海外收入/利润具体数字必须与主营业务数据交叉核对，优先使用 Tushare 结构化数据，
  禁止直接引用未经核对的搜索摘要数字；材料中区分境外收入口径与出口量口径时，明写"年报口径"vs"快报口径"
- **海外业务表述规则**：当材料同时包含"境外收入占比"和"境外利润占比（按毛利倒算）"时，
  必须合并表述为**"境外收入占比从X%跃升至Y%，按毛利倒算利润占比约Z%"**
  （举例：境外收入占比38.65%、上年28.6%，利润占比42.4%时写
  "境外收入占比从28.6%跃升至38.65%，按毛利倒算利润占比约42.4%"），
  禁止分开写"收入占比X%，利润占比Y%"以避免读者混淆两个口径
- 「分析局限性说明」中必须增加：
  ① 模型历史方向判断命中率约50%，结论因此偏保守
  ② 机构预测与实际的预期差风险（如"机构预测2026年全年EPS均值4.54元，隐含净利润约413.97亿，
     Q1实际仅40.85亿，后三季度需达成约373亿（环比Q1增长约410%+），预期差极大"）"""
...
_INDUSTRY_RULES = """
【产业链/行业类问题的额外结构要求】
- 报告章节标题必须严格按以下**标准化7节结构**使用 `##` 标题：
   `## 📌 结论与操作` → `## 产业链全景图` → `## 关键环节深度分析` →
   `## 候选公司详情` → `## 行业趋势与催化时间轴` → `## 风险提示` →
   `## 数据准确性声明` → `## 分析局限性说明`
...
- 必须放在最前面，压缩为10行以内，包含：
  ...
  - **综合排名**：引用调整后综合分和名次，标注象限（机会区/拥挤区/危险区/中性）
  - **方向与操作**：偏多/中性/偏空 + 不介入的原因或回踩观察位（双重校验）
  - **核心逻辑**：一句话含关键数字
  - **最大风险**：一句话
  - **自动盯梢**：含重估触发条件，说明系统会自动盯梢、命中会推送提醒；
    结论为"不参与"时表述为"暂不参与，观察池+触发条件在盯"

**第2节 产业链全景图**
- 产业链上中下游结构全景（按上游/中游/下游/特精专新分节，每节用表格列出）
- 环节完整度自检：共覆盖12个标准环节中的几个、缺了哪几个、已自动补充的环节
- 说明候选池全貌：共筛出几家、详细分析了哪几家、取舍标准是什么
- 排名引用规则：材料的综合排名含「调整后综合分」时以调整后分数和名次为准

**第3节 关键环节深度分析**
- 环节利润迁移判断（未来2-4个季度最受益环节+依据）
- 技术路径博弈（各路线定位/份额变化/卡脖子环节）
- 量产进度→业绩兑现映射表

**第4节 候选公司详情（核心差异内容）**
- 每只候选公司用一个`###`小节输出，每只2-3页级别：
  - **公司概况与业务拆解**（主营构成数据、出货量/产能、客户集中度）
  - **财务预测表**（3年一致预期：营收/净利/增速/Forward PE/机构覆盖数）
  - **估值锚**（当前PE+PE分位+双阈值预警+估值敏感性矩阵）
  - **回踩观察位（双重校验）**（技术位+基本面锚+校准取保守值）
  - **催化跟踪**（针对该标的的三级催化事件）
- 特别关注特精专新企业的不可替代性

**第5节 行业趋势与催化时间轴**
- 行业近况与重大事件（近1-3个月已发生的行情驱动，带日期与出处）
- 催化事件分级体系（未来3-6个月）：按一级β/二级α/三级个股分级，
  每条含落地概率/影响幅度/兑现时间窗/出处
- 材料含「行业指数表现」时必须引用其原数
...
**附加收尾**：
- 报告末尾必须包含「⭐ 最值得投资标的」小节（TOP1-3，按综合排名+技术面选）"""
...
class ResponderAgent:
    ...
    def _build_system_prompt(state: AgentState) -> str:
        ...
        prompt = _COMMON_RULES.format(today=date.today().strftime('%Y-%m-%d'),
                                      ...
                                      number_accuracy_rule=NUMBER_ACCURACY_RULE)
    ...
    def _build_title(stock_code: str, stock_type: str,
        ...
        if industry_name or ("," in stock_code):
            ...
            # industry_name 为空或包含股代码（如 LLM 没解析出来）时，从代码反查行业
            if (not ind or re.search(r'\d{6}', ind)) and "," in stock_code:
                try:
                    from collections import Counter
                    from tools.data_router import get_stock_industry
                    codes = [c.strip() for c in stock_code.split(",") if c.strip()]
                    inds = [get_stock_industry(c) for c in codes]
                    inds = [i for i in inds if i]
                    if inds:
                        ind = Counter(inds).most_common(1)[0][0]  # 取众数行业
                except Exception:
                    pass
            if not ind:
                ind = stock_code
