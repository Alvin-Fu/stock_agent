...
5. **产业链地位**：位置、议价力、竞争格局
6. **利好与利空分析**：汇总+影响力(高/中/低)+占比+综合偏多/偏空

【输出要求】每个维度给出明确结论；业务数据尽可能用数字；不足标注「信息不足」；利好利空分析给出占比；最后 3 句以内核心总结"""
...
请按六段式结构输出：一、全景筛选（含资金偏好标签） → 二、业务拆解与经营数据（收入毛利占比/出货量/资本开支/新增订单/技术突破） → 三、基本面评分 → 四、护城河分析（重点关注特精专新） → 五、综合排名 → 六、JSON 候选代码清单"""),
        ]
